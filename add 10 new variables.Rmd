...
3.1 add new variables:"CHCSCNCR", "CHCOCNCR", "CHCCOPD1",
                           "CHILDREN", "VETERAN3", "_RFBMI5", "DIFFWALK", "BLIND", "USEEQUIP","QLACTLM2"   related to "SKIN_CANCER", "OTHER_CANCER", "PULMONARY_DISEASE",
                          "CHILDREN", "VETERAN", "FAT", "DIFFICULT_WALK", "BLIND", "SPECIAL_EQUIPMENT", "ACT_LIMIT"
```{r}

clean_dataset <- X2015[, c("SEX", "_AGE80", "WEIGHT2", "HEIGHT3", "_RFSMOK3", "_RFBING5", "INCOME2",
                           "EXERANY2", "CHCKIDNY", "HAVARTH3", "TOLDHI2", "_RFHYPE5", 
                           "ASTHMA3", "MENTHLTH", "_RACE", "EDUCA", "FRUIT1", "FVBEANS", "FVGREEN",
                           "STRENGTH", "_MICHD",  "CHCSCNCR", "CHCOCNCR", "CHCCOPD1",
                           "CHILDREN", "VETERAN3", "_RFBMI5", "DIFFWALK", "BLIND", "USEEQUIP","QLACTLM2")]


clean_dataset <- subset(clean_dataset, 
                        `_MICHD` %in% c("1", "2") &
                        WEIGHT2 >= 0 & WEIGHT2 <= 999 &
                        HEIGHT3 >= 200 & HEIGHT3 <= 711 &
                        `_RFSMOK3` %in% c("1", "2") &
                        `_RFBING5` %in% c("1", "2") &
                        ...
                        CHCOCNCR %in% c("1", "2") & 
                        CHCCOPD1 %in% c("1", "2") & 
                        !(CHILDREN %in% c(99)) & !is.na(CHILDREN) & 
                        VETERAN3 %in% c("1", "2") & 
                        `_RFBMI5` %in% c("1", "2") & 
                        DIFFWALK %in% c("1", "2") & 
                        BLIND %in% c("1", "2") & 
...
names(clean_dataset) <- c("SEX", "AGE", "WEIGHT", "HEIGHT", "SMOK", "DRINK", "INCOME",
                          ...
                          "DARK_GREEN_VEG", "MUSCLES_EXERCISE", "y",
                           "SKIN_CANCER", "OTHER_CANCER", "PULMONARY_DISEASE",
                          "CHILDREN", "VETERAN", "FAT", "DIFFICULT_WALK", "BLIND", "SPECIAL_EQUIPMENT", "ACT_LIMIT")
...
clean_dataset$SKIN_CANCER <- factor(clean_dataset$SKIN_CANCER, levels = c(1, 2), labels = c("_YES", "_NO"))
clean_dataset$OTHER_CANCER <- factor(clean_dataset$OTHER_CANCER, levels = c(1, 2), labels = c("_YES", "_NO"))
clean_dataset$PULMONARY_DISEASE <- factor(clean_dataset$PULMONARY_DISEASE, levels = c(1, 2), labels = c("_YES", "_NO"))
clean_dataset$VETERAN <- factor(clean_dataset$VETERAN, levels = c(1, 2), labels = c("_YES", "_NO"))
clean_dataset$FAT <- factor(clean_dataset$FAT, levels = c(1, 2), labels = c("_NO", "_YES"))
clean_dataset$DIFFICULT_WALK <- factor(clean_dataset$DIFFICULT_WALK, levels = c(1, 2), labels = c("_YES", "_NO"))
clean_dataset$BLIND <- factor(clean_dataset$BLIND, levels = c(1, 2), labels = c("_YES", "_NO"))
clean_dataset$SPECIAL_EQUIPMENT <- factor(clean_dataset$SPECIAL_EQUIPMENT, levels = c(1, 2), labels = c("_YES", "_NO"))
clean_dataset$ACT_LIMIT <- factor(clean_dataset$ACT_LIMIT, levels = c(1, 2), labels = c("_YES", "_NO"))
...
clean_dataset$CHILDREN[clean_dataset$CHILDREN == 88] <- 0  # 88 -> 0
...
```
5.2 Data set division


The dataset (sampled_data) is split into:
70% training set (train_data)
30% test set (test_data)
...
big_model_no_interaction <- glm(
  y ~ SEX + AGE + WEIGHT + HEIGHT + RACE + EDUCATION + INCOME +
       ...
       HIGH_BLOOD_PRESSURE + ASTHMA + STROKE + KIDNEY + HIGH_CHOLESTEROL + MENTAL + ARTHRITIS +SKIN_CANCER + OTHER_CANCER + PULMONARY_DISEASE + CHILDREN + VETERAN + FAT + DIFFICULT_WALK + BLIND + SPECIAL_EQUIPMENT + ACT_LIMIT,
...
optimal_model <- stepAIC(big_model_no_interaction, direction = "both")
...
optimal_model_s <- stepAIC(big_model_no_interaction_s, direction = "both")
...
no interaction logistic model smote
```{r}
# Compute Youden's index for logistic model with interaction
logit_pred_prob_i <- predict(big_model_no_interaction_s, newdata = test_data, type = "response")
...
no interaction logistic model STEPAIC
```{r}
# Compute Youden's index for logistic model without interaction using SMOTE
logit_pred_prob_s <- predict(optimal_model, newdata = test_data, type = "response")
logit_pred_prob_s <- as.numeric(logit_pred_prob_s)
roc_obj_s <- roc(as.numeric(test_data$y) - 1, logit_pred_prob_s)
auc_value_s <- auc(roc_obj_s)
best_coords_s <- coords(roc_obj_s, "best", ret = c("threshold", "specificity", "sensitivity"), best.method = "youden")
print(best_coords_s)
best_threshold_s <- as.numeric(best_coords_s["threshold"])

# Classify predictions using the optimal threshold
#logit_pred_class_s <- ifelse(logit_pred_prob_s > best_threshold_s, "yes", "no")
logit_pred_class_s <- ifelse(logit_pred_prob_s > 0.5, "yes", "no")
logit_pred_class_s <- factor(logit_pred_class_s, levels = c("no", "yes"))

# Compute confusion matrix
cm_s <- confusionMatrix(logit_pred_class_s, test_data$y, positive = "yes")

# Extract evaluation metrics
accuracy_s <- cm_s$overall["Accuracy"]
precision_s <- cm_s$byClass["Precision"]
recall_s <- cm_s$byClass["Recall"]
f1_score_s <- 2 * (precision_s * recall_s) / (precision_s + recall_s)
npv_s <- cm_s$byClass["Neg Pred Value"]
ppv_s <- precision_s  # PPV is Precision

# Compute MSE, MAE, RMSE
mse_s <- mean((as.numeric(test_data$y) - 1 - logit_pred_prob_s)^2)
mae_s <- mean(abs(as.numeric(test_data$y) - 1 - logit_pred_prob_s))
rmse_s <- sqrt(mse_s)

# Print metrics
metrics_s <- list(
  Accuracy = as.numeric(accuracy_s),
  Precision = as.numeric(precision_s),
  Recall = as.numeric(recall_s),
  F1_Score = as.numeric(f1_score_s),
  AUC = as.numeric(auc_value_s),
  PPV = as.numeric(ppv_s),
  NPV = as.numeric(npv_s),
  MSE = as.numeric(mse_s),
  MAE = as.numeric(mae_s),
  RMSE = as.numeric(rmse_s)
)
...
# Explicitly print MSE, MAE, RMSE separately
cat("\nMean Squared Error (MSE):", mse_s, "\n")
cat("Mean Absolute Error (MAE):", mae_s, "\n")
cat("Root Mean Squared Error (RMSE):", rmse_s, "\n")
...
no interaction logistic model STEPAIC SMOTE
```{r}
# Compute Youden's index
logit_pred_prob <- predict(optimal_model_s, newdata = test_data, type = "response")
logit_pred_prob <- as.numeric(logit_pred_prob)
...
```{r}
# Logistic Regression
logit_pred_prob_s <- predict(big_model_no_interaction_s, newdata = test_data, type = "response")
